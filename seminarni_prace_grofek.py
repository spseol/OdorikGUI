...
def datum_hovoru_2hod(datum):   # Při letním čase, stačí přejmenovat funkci
                                # (2hod na 1hod a v druhé funkci naopak)
    ...
    pred_dvema_hodinama = "%s.%s.%s  %s" % (str(pred_dvema_hodinama)[8:10],
                                            str(pred_dvema_hodinama)[5:7],
                                            str(pred_dvema_hodinama)[:4],
                                            str(pred_dvema_hodinama)[11:19])
...
def aktualizovat_posledni_hovory(prihl_jmeno, heslo, posl_hov_datum1,
                                 posl_hov_cislo1, posl_hov_cena1,
                                 pred_posl_hov_datum1, pred_posl_hov_cislo1,
                                 pred_posl_hov_cena1,
                                 pred_pred_posl_hov_datum1,
                                 pred_pred_posl_hov_cislo1,
                                 pred_pred_posl_hov_cena1, kontakty_okno):
    try:
        ...
        posl_hovor_mezikrok2 = json.loads(posl_hovor_mezikrok,
                                          object_hook=datum_cislo_cena)
        try:
    ...
    kontakty_okno.after(10000,
                        lambda: aktualizovat_posledni_hovory(prihl_jmeno,
                                                             heslo,
                                                             posl_hov_datum1,
                                                             posl_hov_cislo1,
                                                             posl_hov_cena1,
                                                             pred_posl_hov_datum1,
                                                             pred_posl_hov_cislo1,
                                                             pred_posl_hov_cena1,
                                                             pred_pred_posl_hov_datum1,
                                                             pred_pred_posl_hov_cislo1,
                                                             pred_pred_posl_hov_cena1,
                                                             kontakty_okno))
...
def aktualizovat_kontakty(udaje, listbox, kontakty_okno, zkratky_jmena_cisla):
    ...
    try:
        ...
        kontakty_dalsi_mezikrok2 = kontakty_mezikrok2.read()
        kontakty2 = json.loads(kontakty_dalsi_mezikrok2, object_hook=vypsat)
    # kontakty jako 3-členné ntice v seznamu
        ...
        for i in kontakty2:
            ...
            if citac_dvojtecka > 0:
                ...
                if len(upr_cis) == 9:
                    ...
                    upravene_cislo = "%s %s %s %s" % (i[2][-12:-9],
                                                      i[2][-9:-6],
                                                      i[2][-6:-3],
                                                      i[2][-3:])
            else:
...
def prihlasit():
    ...
    global udaje
    if prihl_jmeno_entry.get() == "" or heslo_entry.get() == "":
        tkm.showwarning("CHYBA", u"Jeden, nebo oba údaje zůstaly nevyplněny")
    udaje = urllib.urlencode({"user": prihl_jmeno, "password": heslo})
    try:
        kredit_mezikrok = urllib.urlopen("https://www.odorik.cz/api/v1/balance?"+udaje)  # získání kreditu
        ...
        if kredit == "error authentication_failed":     #  špatně zadané údaje
            ...
            kredit_label = tk.Label(info_frame,
                                    text="Váš kredit je: "+kredit+" Kč",
                                    bg="#32CD32", font="Arial_black 10 bold")
            kredit_label.grid(sticky=tk.W, column=0, row=0)
            kontakty_okno.after(10000,
                                lambda: aktualizovat_kredit(udaje,
                                                            kontakty_okno,
                                                            kredit_label))
            ...
            posledni_hovor_frame = tk.LabelFrame(info_frame,
                                                 ...
                                                 bg="#32CD32",
                                                 font="Arial_black 8 bold")
            posl_hov_datum = tk.Label(posledni_hovor_frame, text="Datum  Čas",
                                      width=21, bg="#32CD32",
                                      font="Arial_black 10 bold")
            posl_hov_datum.grid(row=2)
            posl_hov_cislo = tk.Label(posledni_hovor_frame, text="Číslo",
                                      width=21, bg="#32CD32",
                                      font="Arial_black 10 bold")
            posl_hov_cislo.grid(row=2, column=1)
            posl_hov_cena = tk.Label(posledni_hovor_frame, text="Cena",
                                     width=21, bg="#32CD32",
                                     font="Arial_black 10 bold")
            posl_hov_cena.grid(row=2, column=2)
            posl_hov_datum1 = tk.Label(posledni_hovor_frame, text="", width=21,
                                       bg="#32CD32", font="Arial_black 8 bold")
            posl_hov_datum1.grid(row=3)
            posl_hov_cislo1 = tk.Label(posledni_hovor_frame, text="", width=21,
                                       bg="#32CD32", font="Arial_black 8 bold")
            posl_hov_cislo1.grid(row=3, column=1)
            posl_hov_cena1 = tk.Label(posledni_hovor_frame, text="", width=21,
                                      bg="#32CD32", font="Arial_black 8 bold")
            posl_hov_cena1.grid(row=3, column=2)
            pred_posl_hov_datum1 = tk.Label(posledni_hovor_frame, text="",
                                            width=21, bg="#32CD32",
                                            font="Arial_black 8 bold")
            pred_posl_hov_datum1.grid(row=4)
            pred_posl_hov_cislo1 = tk.Label(posledni_hovor_frame, text="",
                                            width=21, bg="#32CD32",
                                            font="Arial_black 8 bold")
            pred_posl_hov_cislo1.grid(row=4, column=1)
            pred_posl_hov_cena1 = tk.Label(posledni_hovor_frame, text="",
                                           width=21, bg="#32CD32",
                                           font="Arial_black 8 bold")
            pred_posl_hov_cena1.grid(row=4, column=2)
            pred_pred_posl_hov_datum1 = tk.Label(posledni_hovor_frame, text="",
                                                 width=21, bg="#32CD32",
                                                 font="Arial_black 8 bold")
            pred_pred_posl_hov_datum1.grid(row=5)
            pred_pred_posl_hov_cislo1 = tk.Label(posledni_hovor_frame, text="",
                                                 width=21, bg="#32CD32",
                                                 font="Arial_black 8 bold")
            pred_pred_posl_hov_cislo1.grid(row=5, column=1)
            pred_pred_posl_hov_cena1 = tk.Label(posledni_hovor_frame, text="",
                                                width=21, bg="#32CD32",
                                                font="Arial_black 8 bold")
            pred_pred_posl_hov_cena1.grid(row=5, column=2)
            posledni_hovor_frame.grid(pady=1, columnspan=4,
            ...
            kontakty_okno.after(10000,
                                lambda: aktualizovat_posledni_hovory(prihl_jmeno,
                                                                     ...
                                                                     kontakty_okno))
    # ##########################################################################
            kontakty_frame = tk.LabelFrame(prihl_okno, text="Kontakty",
                                           bg="#32CD32",
                                           font="Arial_black 8 bold")
            callback_podframe = tk.Frame(kontakty_frame, bg="#32CD32")
            callback_podframe.grid(row=0, sticky=tk.E+tk.W+tk.N+tk.S, column=0)
            callback_button = tk.Button(callback_podframe,
                                        text="Objednat callback nyní",
                                        font="Arial_black 8 bold",
                                        activebackground="#99FF99",
                                        bg="#409940", width=25,
                                        command=lambda: objednat_callback(listbox,
                                                                          udaje,
                                                                          kontakty))
            callback_button.grid(row=0, column=0, padx=40, pady=10,
                                 sticky=tk.W)
            spozdeny_callback_button = tk.Button(callback_podframe,
                                                 text="Objednat zpožděný callback",
                                                 font="Arial_black 8 bold",
                                                 activebackground="#99FF99",
                                                 bg="#409940", width=25,
                                                 command=lambda: callback(listbox,
                                                                          kontakty,
                                                                          udaje))
            spozdeny_callback_button.grid(row=0, column=1, sticky=tk.E,
                                          padx=40, pady=10)
            listbox_frame = tk.Frame(kontakty_frame, bg="#32CD32")
            listbox_frame.grid(row=1, column=0, padx=7)
            scrollbar = tk.Scrollbar(listbox_frame, orient=tk.VERTICAL)
            listbox = tk.Listbox(listbox_frame, yscrollcommand=scrollbar.set,
                                 width=85, bg="#99FF99", font="Courier 8 bold",
                                 bd=0)
            ...
            pridat_button = tk.Button(kontakty_buttony_frame,
                                      text="Přidat kontakt",
                                      font="Arial_black 8 bold",
                                      activebackground="#99FF99",
                                      bg="#409940",
                                      command=lambda: pridat_kontakt(udaje,
                                                                     listbox))
            pridat_button.grid(row=0, column=0, padx=20, pady=10)
            odebrat_button = tk.Button(kontakty_buttony_frame,
                                       text="Odebrat kontakt",
                                       command=lambda: odebrat(listbox,
                                                               udaje,
                                                               kontakty),
                                       font="Arial_black 8 bold",
                                       activebackground="#99FF99",
            ...
            upravit_button = tk.Button(kontakty_buttony_frame,
                                       text="Upravit kontakt",
                                       font="Arial_black 8 bold",
                                       activebackground="#99FF99",
                                       bg="#409940",
                                       command=lambda: upravit_kontakt(listbox,
                                                                       udaje,
                                                                       kontakty))
...
def upravit_kontakt(listbox, udaje, kontakty):
    try:  
        ...
        upravit_kontakt_label = tk.Label(upravit_kontakt_frame,
                                         bg="#32CD32", text=u"Upravit kontakt",
                                         font="Arial_black 15 bold")
        upravit_kontakt_label.grid(row=0, columnspan=3)
        zkratka_label = tk.Label(upravit_kontakt_frame, text="Zkratka",
                                 bg="#32CD32", font="Arial_black 10 bold")
        zkratka_label.grid(row=1, column=0)
        upravit_cislo_label = tk.Label(upravit_kontakt_frame, text=u"Číslo",
                                       bg="#32CD32",
                                       font="Arial_black 10 bold")
        upravit_cislo_label.grid(row=1, column=2)
        jmeno_label = tk.Label(upravit_kontakt_frame, text=u"Jméno",
                               bg="#32CD32", font="Arial_black 10 bold")
        ...
        upravit_kontakt_button = tk.Button(upravit_kontakt_frame,
                                           text="Upravit kontakt",
                                           activebackground="#99FF99",
                                           bg="#409940",
                                           font="Arial_black 8 bold",
                                           command=lambda: upravit(kontakt,
                                                                   zkratka_entry,
                                                                   cislo_entry,
                                                                   jmeno_entry,
                                                                   udaje,
                                                                   upravit_kontakt_okno,
                                                                   listbox))
...
def upravit(kontakt, zkratka_entry, cislo_entry, jmeno_entry, udaje,
            upravit_kontakt_okno, listbox):
    zkratka = str(kontakt[0])
    nova_zkratka = zkratka_entry.get()
    nove_jmeno = jmeno_entry.get()
    nove_cislo = cislo_entry.get()
    nove_udaje = urllib.urlencode({"shortcut": nova_zkratka,
                                   "number": nove_cislo, "name": nove_jmeno})
    celkove_udaje = udaje+"&"+nove_udaje
    try:
        upravit = requests.put(url='https://www.odorik.cz/api/v1/speed_dials/'+zkratka+'.json',
                               params=celkove_udaje)
        ...
        if odpoved_mezikrok[2] == "e":
            ...
            for i in odpoved:
                if i == "invalid_shortcut":
                    ...
                    tkm.showerror("CHYBA",
                                  u"Vaše rychlé kontakty jsou již plné")
                elif i == "unauthorized":
...
def pridat_kontakt(udaje, listbox):
    ...
    pridat_kontakt_label = tk.Label(pridat_kontakt_frame, bg="#32CD32",
                                    text=u"Přidat kontakt",
                                    font="Arial_black 15 bold")
    pridat_kontakt_label.grid(row=0, columnspan=3)
    zkratka_label = tk.Label(pridat_kontakt_frame, text="Zkratka",
                             bg="#32CD32", font="Arial_black 10 bold")
    zkratka_label.grid(row=1, column=0)
    pridat_cislo_label = tk.Label(pridat_kontakt_frame, text=u"Číslo",
                                  bg="#32CD32", font="Arial_black 10 bold")
    pridat_cislo_label.grid(row=1, column=2)
    jmeno_label = tk.Label(pridat_kontakt_frame, text=u"Jméno",
                           bg="#32CD32", font="Arial_black 10 bold")
    jmeno_label.grid(row=1, column=1)
    ...
    jmeno_entry.grid(row=2, column=1)
    pridat_kontakt_button = tk.Button(pridat_kontakt_frame,
                                      text="Přidat kontakt",
                                      activebackground="#99FF99", bg="#409940",
                                      font="Arial_black 8 bold",
                                      command=lambda: pridat(zkratka_entry,
                                                             cislo_entry,
                                                             jmeno_entry,
                                                             pridat_kontakt_okno,
                                                             udaje, listbox))
...
def pridat(zkratka_entry, cislo_entry, jmeno_entry, pridat_kontakt_okno,
           udaje, listbox):
    zkratka = zkratka_entry.get()
    cislo = cislo_entry.get()
    jmeno = jmeno_entry.get()
    prihl = httplib.HTTPSConnection("www.odorik.cz")
    dalsi_udaje = urllib.urlencode({'shortcut': zkratka, 'name': jmeno,
                                    'number': cislo})
...
def odebrat(listbox, udaje, kontakty):
    try:  
        ...
        if tkm.askokcancel("Smazat", "Jste si jisti?"):
            try:
                smazat = requests.delete(url='https://www.odorik.cz/api/v1/speed_dials/'+zkratka+'.json',
                                         params=udaje)
            except:
                tkm.showwarning("CHYBA", u"Nelze se připojit k síti")
            else:
...
def nastavit_moje_cislo(moje_cislo_label):
    ...
    nastavit_cislo_entry = tk.Entry(nastavit_cislo, justify="center",
                                    bg="#99FF99", relief="groove")
    nastavit_cislo_entry.grid(row=0, padx=10, pady=10)
    nastavit_cislo_entry.insert(tk.END, moje_ulozene_cislo)
    nastavit_cislo_button = tk.Button(nastavit_cislo,
                                      text=u"Nastavit moje číslo",
                                      font="Arial_black 8 bold",
                                      activebackground="#99FF99", bg="#409940",
                                      command=lambda: ulozit_cislo(nastavit_cislo_entry,
                                                                   nastavit_cislo_okno,
                                                                   moje_cislo_label))
...
def callback(listbox, kontakty, udaje):  # vyvolá okno po kliknutí na tlačítko callb.
    try:
        moje_cislo_soubor = open("moje_cislo.txt", "r")
        ...
        try:  
            ...
            callback_datum_frame = tk.LabelFrame(callback_okno,
                                                 text="Objednat callback na určené datum",
                                                 bg="#32CD32",
                                                 font="Arial_black 8 bold")
            callback_datum_frame.grid(padx=10, pady=5)
            datum_callback_label = tk.Label(callback_datum_frame, text="Datum",
                                            bg="#32CD32",
                                            font="Arial_black 10 bold")
            datum_callback_label.grid(row=0, column=0)
            datum_cas_callback_label = tk.Label(callback_datum_frame,
                                                text=u"Čas", bg="#32CD32",
                                                font="Arial_black 10 bold")
            datum_cas_callback_label.grid(row=0, column=1)
            global datum_callback_entry
            datum_callback_entry = tk.Entry(callback_datum_frame,
                                            justify="center", bg="#99FF99",
                                            relief="groove")
            datum_callback_entry.grid(row=1, column=0)
            datum_callback_entry.insert(tk.END, "YYYY-MM-DD")
            global datum_cas_callback_entry
            datum_cas_callback_entry = tk.Entry(callback_datum_frame,
                                                justify="center", bg="#99FF99",
                                                relief="groove")
            datum_cas_callback_entry.grid(row=1, column=1)
            datum_cas_callback_entry.insert(tk.END, "HH:MM:SS")
            datum_callback_button = tk.Button(callback_datum_frame,
                                              activebackground="#99FF99",
                                              text="Objednat dle data",
                                              font="Arial_black 8 bold",
                                              bg="#409940",
                                              width=25,
                                              command=lambda: callback_datum(listbox,
                                                                             udaje,
                                                                             datum_callback_entry,
                                                                             datum_cas_callback_entry,
                                                                             callback_hlavni_okno, kontakty,
                                                                             komu_volat))
            datum_callback_button.grid(row=2, columnspan=2, padx=5, pady=10)
            callback_cas_frame = tk.LabelFrame(callback_okno,
                                               text="Objednat callback na určený čas",
                                               bg="#32CD32",
                                               font="Arial_black 8 bold")
            callback_cas_frame.grid(row=1, pady=10)
            minuty_callback_label = tk.Label(callback_cas_frame,
                                             text=u"Zpoždění (minuty)",
                                             bg="#32CD32",
                                             font="Arial_black 10 bold")
            minuty_callback_label.grid(row=0, column=0, columnspan=2)
            global minuty_callback_entry
            minuty_callback_entry = tk.Entry(callback_cas_frame, width=10,
                                             justify="center", bg="#99FF99",
                                             relief="groove")
            minuty_callback_entry.grid(row=1, column=0, columnspan=2)
            minuty_callback_entry.insert(tk.END, "1")
            minuty_callback_button = tk.Button(callback_cas_frame,
                                               activebackground="#99FF99",
                                               text="Objednat dle zpoždění",
                                               font="Arial_black 8 bold",
                                               bg="#409940",
                                               width=25,
                                               command=lambda: callback_cas(listbox,
                                                                            udaje,
                                                                            callback_hlavni_okno,
                                                                            minuty_callback_entry,
                                                                            kontakty,
                                                                            komu_volat))
...
def callback_cas(listbox, udaje, callback_hlavni_okno, minuty_callback_entry,
    ...
    try:
        objednat_callback = urllib.urlopen("""https://www.odorik.cz/api/v1/callback""",
                                           vsechny_udaje)
        ...
        if odpoved == "successfully_enqueued":
            ...
            tkm.showwarning("Callback",
                            u"Požadavek se nepodařilo odeslat na Odorik.cz")
...
def callback_datum(listbox, udaje, datum_callback_entry,
                   datum_cas_callback_entry, callback_hlavni_okno,
                   kontakty, volane_cislo):
    moje_cislo_soubor = open("moje_cislo.txt", "r")
    moje_cislo = moje_cislo_soubor.readline()
    moje_cislo_soubor.close()
    ...
    try:
        ...
        if odpoved == "successfully_enqueued":
            ...
            tkm.showwarning("CHYBA",
                            u"Datum nebo čas zadáno ve špatném formátu")
...
prihl_nadpis_label = tk.Label(hl_okno, text="Příhlášení",
                              font="Arial_black 15 bold", bg="#32CD32")
prihl_nadpis_label.grid()

jmeno_frame = tk.Frame(hl_okno, bg="#32CD32", pady=10)
jmeno_frame.grid(row=1)
prihl_jmeno_label = tk.Label(jmeno_frame, text="Jméno", bg="#32CD32",
                             font="Arial_black 10 bold")
...
heslo_label = tk.Label(hl_okno, text="Heslo", bg="#32CD32",
                       font="Arial_black 10 bold")
...
ulozit_udaje_check = tk.Checkbutton(hl_okno, text="Uložit údaje",
                                    variable=ulozit_udaje_promenna)
ulozit_udaje_check["bg"] = "#32CD32"
ulozit_udaje_check["font"] = "Arial_black 8 bold"
ulozit_udaje_check.grid(row=4, pady=2)

prihlaseni_button = tk.Button(hl_okno, text="Přihlásit", bg="#409940",
                              command=prihlasit, font="Arial_black 8 bold",
                              activebackground="#99FF99")
